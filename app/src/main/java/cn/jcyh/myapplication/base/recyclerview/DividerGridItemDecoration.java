package cn.jcyh.myapplication.base.recyclerview;
...
//            mDivider.setBounds(left, top, right, bottom);
//            mDivider.draw(c);
//            Timber.e("-------lefta:" + left + "---" + top + "--" + right + "---" + bottom);
    ...
    private boolean isfirstRow(RecyclerView parent, int pos, int spanCount,
        ...
        if (layoutManager instanceof GridLayoutManager) {
            // childCount = childCount - childCount % spanCount;
            int lines = childCount % spanCount == 0 ? childCount / spanCount : childCount /
                    spanCount + 1;
            //如是第一行则返回true
...
//        } else {
        if (isfirstRow(parent, itemPosition, spanCount, childCount)) {
            outRect.set(0, (spanCount - 1) * mDivider.getIntrinsicWidth() / spanCount, mDivider
                            .getIntrinsicWidth(),
